...
class Parser:
    ...
    def summa(self):
        n = self.term()
        while self.lexer.sym == Token.PLUS or self.lexer.sym == Token.MINUS:
            kind = NodeType.ADD if self.lexer.sym == Token.PLUS else NodeType.SUB
            self.lexer.next_token()
    ...
    def test(self):
        ...
        if self.lexer.sym == Token.LESS:
            self.lexer.next_token()
            n = Node(NodeType.LT, op1=n, op2=self.summa())
        return n
